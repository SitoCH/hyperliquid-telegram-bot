...
def save_to_buffer(
    ...
    resistance_levels, support_levels = find_significant_levels(
        df, wyckoff, mid, timeframe
    )
    ...
    buf = io.BytesIO()
    fig, ax = plt.subplots(
        3, 1, figsize=(10, 6), gridspec_kw={"height_ratios": [7, 1, 1]}
    )

    df_plot["SuperTrend_Green"] = np.where(
        df_plot["c"] > df_plot["SuperTrend"], df_plot["SuperTrend"], np.nan
    )
    df_plot["SuperTrend_Red"] = np.where(
        df_plot["c"] <= df_plot["SuperTrend"], df_plot["SuperTrend"], np.nan
    )

    if use_heikin_ashi is None:
        use_heikin_ashi = _env_bool("HTB_USE_HEIKIN_ASHI", False)
    candle_df = heikin_ashi(df_plot) if use_heikin_ashi else df_plot
    ...
    for level in reversed(resistance_levels):
        ...
        level_lines.append(
            mpf.make_addplot(
                ...
                color="purple",
                width=0.5,
                label=f"R {fmt_price(level)} ({pct_str})",
                linestyle="--",
            )
        )

    is_bullish = candle_df["c"].iloc[-1] >= candle_df["o"].iloc[-1]
    ...
    for level in support_levels:
        ...
        level_lines.append(
            mpf.make_addplot(
                ...
                color="purple",
                width=0.5,
                label=f"S {fmt_price(level)} ({pct_str})",
                linestyle=":",
            )
        )
