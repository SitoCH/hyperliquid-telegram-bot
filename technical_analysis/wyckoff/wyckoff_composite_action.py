...
def detect_composite_action(
    ...
    try:
        ...
        liquidation_cascade = (
            abs(recent_change) > max(0.05, price_volatility * 4)  # Adaptive to market conditions
            and abs(vol_metrics.strength) > 2.5
            and vol_metrics.ratio > 3.0
            and body_to_range_ratio > 0.8  # Strong directional candle
        )
        ...
        absorption_volume = (
            vol_metrics.ratio > 2.0                      # Double normal volume
            and abs(vol_metrics.strength) > 1.5              # Significant volume deviation
            and body_to_range_ratio < 0.3                    # Small body relative to range
            and volume_trend > 1.2                               # Rising volume pattern - using volume_trend
        )
        ...
        absorption = (
            relative_range > 1.2  # Wider than normal range
            and abs(price_close.iloc[-1]) < min_price_move * 0.7  # Small body relative to volatility
            and vol_metrics.consistency > 0.6  # Consistent volume pattern
        )
        ...
        bullish_divergence = (
            price_lows.iloc[-1] < price_lows.iloc[-5] * 0.995  # Lower low (with small threshold)
            and volume_trend > 1.5                                # Higher volume - using volume_trend
            and effort_vs_result > 0.2                             # Positive effort vs result
            and df['c'].iloc[-1] > df['o'].iloc[-1]                # Bullish candle
            and abs(df['c'].iloc[-1] - df['o'].iloc[-1]) > min_price_move  # Significant price move
        )

        # Bearish divergence with consistent use of min_price_move and volume_trend
        bearish_divergence = (
            price_highs.iloc[-1] > price_highs.iloc[-5] * 1.005  # Higher high (with small threshold)
            and volume_trend > 1.5                                  # Higher volume - using volume_trend
            and effort_vs_result < -0.2                              # Negative effort vs result
            and df['c'].iloc[-1] < df['o'].iloc[-1]                  # Bearish candle
            and abs(df['c'].iloc[-1] - df['o'].iloc[-1]) > min_price_move  # Significant price move
        )
