...
def detect_spring_upthrust(df: pd.DataFrame, timeframe: Timeframe, idx: int, vol_metrics: VolumeMetrics) -> tuple[bool, bool]:
    ...
    try:
        ...
        is_spring = (
            current_low < low_point         # Makes new low
            and close > low_point               # Closes above the low
            and spring_bounce > price * spring_threshold  # Significant bounce
            and close > current_low * 1.005        # Closes meaningfully above the low
        )

        # Upthrust detection - more precise conditions
        upthrust_rejection = abs(current_high - close) if current_high > high_point else 0
        is_upthrust = (
            current_high > high_point       # Makes new high
            and close < high_point              # Closes below the high
            and upthrust_rejection > price * upthrust_threshold  # Significant rejection
            and close < current_high * 0.995       # Closes meaningfully below the high
        )
...
def detect_wyckoff_phase(df: pd.DataFrame, timeframe: Timeframe, funding_rates: List[FundingRateEntry]) -> WyckoffState:
    ...
    try:
        ...
        phase_vwap_aligned = (
            (current_phase in [WyckoffPhase.MARKUP, WyckoffPhase.ACCUMULATION] and vwap_bias >= 0)
            or (current_phase in [WyckoffPhase.MARKDOWN, WyckoffPhase.DISTRIBUTION] and vwap_bias <= 0)
            or current_phase == WyckoffPhase.RANGING
        )

        # RSI extreme check - overbought in markup or oversold in markdown increases certainty
        rsi_confirms_phase = (
            (current_phase == WyckoffPhase.MARKUP and 40 < rsi_value < 80)
            or (current_phase == WyckoffPhase.MARKDOWN and 20 < rsi_value < 60)
            or (current_phase in [WyckoffPhase.ACCUMULATION, WyckoffPhase.DISTRIBUTION, WyckoffPhase.RANGING])
        )
...
def identify_wyckoff_phase(
    ...
    is_liquidation = (
        abs(recent_change) > price_threshold
        and volume_impulse > vol_threshold
        and price_velocity > velocity_threshold
        and len(effort_vs_result) > 0
        and abs(effort_vs_result.iloc[-1]) > effort_threshold
    )
