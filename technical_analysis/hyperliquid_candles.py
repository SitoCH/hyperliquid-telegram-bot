...
async def analyze_candles_for_coin(context: ContextTypes.DEFAULT_TYPE, coin: str, always_notify: bool) -> None:
    ...
    try:
        ...
        candles_15m = get_candles_with_cache(coin, Timeframe.MINUTES_15, now, 100, hyperliquid_utils.info.candles_snapshot)
        if len(candles_15m) == 0:
            logger.warning(f"Insufficient candles for technical analysis on {coin}")
            return

        candles_30m = get_candles_with_cache(coin, Timeframe.MINUTES_30, now, 200, hyperliquid_utils.info.candles_snapshot)
        candles_1h = get_candles_with_cache(coin, Timeframe.HOUR_1, now, 350, hyperliquid_utils.info.candles_snapshot)
