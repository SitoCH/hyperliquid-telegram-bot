...
class LLMPromptGenerator:
    ...
    def generate_prediction_prompt(
        ...
        prompt_parts.extend([
            ...
            "5. TECHNICAL INDICATORS: Analyze all provided indicators including RSI, MACD, SuperTrend, Stochastic, Williams %R, CCI, ROC, ATR for confluence",
            "6. BOLLINGER BANDS: Price position relative to bands (upper/middle/lower), squeeze/expansion patterns, BB width analysis",
            "7. ICHIMOKU CLOUD: Price above/below cloud, Tenkan/Kijun cross, cloud twist potential, Chikou Span positioning",
            "8. OSCILLATOR ALIGNMENT: Are RSI, Stochastic, Williams %R showing same direction and confirming momentum?",
            "9. EMA/VWAP POSITIONING: Price relative to EMA and VWAP for trend confirmation",
            "10. FIBONACCI LEVELS: Price interaction with key Fib retracements (23.6%, 38.2%, 50%, 61.8%, 78.6%)",
            "11. PIVOT POINTS: Price position relative to daily pivots, resistance/support levels (R1/R2, S1/S2)",
            "12. FUNDING PRESSURE: Current funding rate creating buy/sell pressure?",
            "13. WYCKOFF PHASE: Accumulation, Markup, Distribution, or Markdown based on price-volume relationship?",
            "",
            ...
            "- MANDATORY: Always provide stop_loss and target_price for ALL signals (buy/sell/hold)",
            "- If no clear trade setup exists, use technical levels for defensive TP/SL",
            "",
            "=== RESPONSE FORMAT ===",
            "Provide your analysis in JSON format with complete trade setup details:",
            "CRITICAL: ALL fields below are MANDATORY - never leave stop_loss or target_price empty:",
            "",
            "{",
            ...
            '    "reason": "Concise rationale - REQUIRED for buy/sell signals only",',
            '    "entry_strategy": "Market/limit order strategy - REQUIRED for buy/sell signals only",',
            '    "risk_reward_ratio": 2.5,',
            '    "invalidation_level": "Price level where analysis becomes invalid - REQUIRED for buy/sell only",',
            '    "session_timing": "Optimal entry timing - REQUIRED for buy/sell signals only"',
            '  }',
            "}",
            "",
            "EXECUTION RULES:",
            ...
            "- CRITICAL: NEVER submit a response without stop_loss and target_price values",
            "- For uncertain setups: Use HOLD signal with defensive TP/SL based on nearest S/R levels",
            "- For directional bias without setup: HOLD with TP/SL in direction of bias",
            "",
            "MANDATORY REQUIREMENTS FOR BUY/SELL SIGNALS:",
            "1. ENTRY PRICE: Within 0.5% of current price for actionable trades",
            "2. STOP LOSS: ALWAYS required - logical technical level (S/R, Fibonacci, pivot, ATR-based)",
            "3. TARGET PRICE: ALWAYS required - next key level or measured move target",
            "4. RISK/REWARD: Minimum 1:1.5 ratio required",
            "5. ACTION FIELD: Must match signal (buy=long, sell=short)",
            "6. ENTRY STRATEGY: Required - specify order type and timing",
            "7. INVALIDATION LEVEL: Always required - price where trade setup fails",
            "",
            "REQUIREMENTS FOR HOLD SIGNALS:",
            "1. ENTRY PRICE: Current market price",
            "2. STOP LOSS: Defensive level based on nearest S/R or ATR",
            "3. TARGET PRICE: Next significant technical level",
            "4. ACTION FIELD: 'none' (no position change)",
            "5. ENTRY STRATEGY: 'wait' or 'monitor' - no immediate action needed",
            "",
            "HOLD SIGNAL REQUIREMENTS:",
            "- When no clear trade setup exists, provide HOLD with defensive TP/SL levels",
            "- Stop loss: Nearest major support/resistance or 2-3% ATR-based level",
            "- Target price: Next significant technical level in likely direction",
            "- Entry price: Current market price",
            "- Action: 'none' (indicating no position change)",
            "",
            "HANDLING DIRECTIONAL BIAS WITHOUT CLEAR SETUP:",
            "- If analysis suggests bullish/bearish but no immediate entry: Use HOLD signal",
            "- Provide defensive TP/SL based on key technical levels in the suggested direction",
            "- Example: Bullish bias but no entry → HOLD with SL at nearest support, TP at next resistance",
            "- Example: Bearish bias but no entry → HOLD with SL at nearest resistance, TP at next support",
            "- Always explain why no immediate trade despite directional view",
            "",
