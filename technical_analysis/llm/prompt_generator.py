...
class LLMPromptGenerator:
    ...
    def generate_prediction_prompt(
        ...
        prompt_parts = [
            ...
            "",
            "INSTRUCTION: Analyze current market state and provide actionable trading recommendations for 4-6 hour position holding.",
        ...
        prompt_parts.extend([
            ...
            "FOCUS: Provide actionable trading insights for immediate entry with 4-6 hour holding potential.",
            "",
            "KEY ANALYSIS AREAS:",
            ...
            "12. FUNDING PRESSURE: Current funding rate creating long/short pressure?",
            "13. WYCKOFF PHASE: Accumulation, Markup, Distribution, or Markdown based on price-volume relationship?",
            "",
            "TRADING REQUIREMENTS:",
            "- Recommend LONG/SHORT only if there's a clear setup actionable within 5-30 minutes",
            "- Target 4-6 hour position holding period for optimal risk/reward",
            "- Minimum 1:1.5 risk/reward ratio required",
            "- MANDATORY: Always provide stop_loss and target_price for ALL signals",
            "- Stop loss and target price must be at least 1.5% away from current price",
            "- If technical levels are closer than 1.5%, use the next significant level beyond this threshold",
            "- For uncertain markets: Use HOLD with defensive stop/target based on key technical levels",
            "",
            "=== RESPONSE FORMAT ===",
            ...
            '    "entry_strategy": "Market/limit order strategy - REQUIRED for long/short signals only",',
            '    "risk_reward_ratio": 2.5,',
            '    "invalidation_level": "Price level where analysis becomes invalid - REQUIRED for long/short only",',
            '    "session_timing": "Optimal entry timing - REQUIRED for long/short signals only"',
            '  }',
            "}",
            "",
            "EXECUTION RULES:",
            "- IMMEDIATE ACTION: Only recommend LONG/SHORT if setup is actionable within 5-30 minutes",
            "- CURRENT FOCUS: Signal based on what's happening NOW, not future possibilities", 
            "- POSITION PRIORITY: Enter quickly on good setups, hold positions for 4-6 hours with clear exits",
            ...
            "- TARGET PRICE: Next significant technical level in anticipated direction (minimum 1.5% away)",
